...
async function handler(m, { conn, args }) {
  ...
  if (!city) {
    return conn.sendMessage(
      m.chat,
      { text: "⚠️ Inserisci il nome di una città.\nEsempio: .calendario Roma" },
      { quoted: m }
    );
  }
  ...
  try {
    ...
    return conn.sendMessage(
      m.chat,
      { text: "❌ Impossibile recuperare i dati meteo per la città specificata." },
      { quoted: m }
    );
  }
  ...
  if (Number(giorno) === pasqua.getDate() && meseCorrente === pasqua.getMonth() + 1) {
    festivita = "Santa Pasqua 🕊️";
  } else if (Number(giorno) === pasquetta.getDate() && meseCorrente === pasquetta.getMonth() + 1) {
