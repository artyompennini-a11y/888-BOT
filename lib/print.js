...
export default async function (m, conn = { user: {} }) {
  ...
  try {
    ...
    const isAdmin = isGroup ? checkAdmin(groupMeta, senderJid, conn) : false;
    ...
    const righe = [
      ...
      `${c.label('UTENTE')} ⇢ ${c.text(sender)}${isGroup ? ` ${c.gray('⟡ ' + getUserStatus(isOwner, isAdmin, isPremium, isBanned, c))}` : ''}`,
...
function checkAdmin(groupMeta, senderId, conn) {
  try {
