...
const handler = async (message, { conn, usedPrefix }) => {
  if (global.conn.user.jid !== conn.user.jid) {
    ...
    return true; // Ritorna true per evitare 'undefined' nel terminale
  }
  ...
  // Recupero sicuro del buffer dell'immagine
  let imgBuffer = null;
  try {
    ...
    imgBuffer = Buffer.alloc(0); // Buffer vuoto di fallback in caso di errore di rete
  }
  ...
  return true; // Risolve la stampa di 'undefined' a fine esecuzione
};
