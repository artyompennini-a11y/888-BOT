...
async function saveDB(db){
  pokemonSaveLock=true
  try{
    await fs.mkdir(path.dirname(DB_PATH),{recursive:true})
    await fs.writeFile(DB_PATH,JSON.stringify(db,null,2))
  }finally{ pokemonSaveLock=false }
}
...
async function fetchPokemon(nameOrId){
  ...
  return {
    ...
    moves:data.moves.sort(()=>Math.random()-0.5).slice(0,4).map(m=>m.move.name.replace(/-/g," ").toUpperCase()),
...
function buildCard(poke,dataC=null){
  ...
  const movesStr=poke.moves.map((m,i)=>`• ${["①","②","③","④"][i]} ${m}`).join("\n")
...
`.trim())
    ...
    let poke
    try{ poke=await fetchPokemon(trovato.id) }
    catch{ return m.reply("❌ Errore nel caricare i dati.") }
