...
async function handler(m, { isBotAdmin, isOwner, text, conn }) {
  ...
  if (!target || target === sender || target === normalize(conn.user?.jid || conn.user?.id)) {
    return m.reply('ⓘ 𝐌𝐞𝐧𝐳𝐢𝐨𝐧𝐚 𝐥𝐚 𝐩𝐞𝐫𝐬𝐨𝐧𝐚 𝐝𝐚 𝐫𝐢𝐦𝐮𝐨𝐯𝐞𝐫𝐞')
  }
  ...
  if (admin) return m.reply(`> ⚠️ 𝐀𝐧𝐭𝐢-𝐊𝐢𝐜𝐤\n> ⓘ 𝐋'𝐮𝐭𝐞 n𝐭𝐞 𝐜𝐡𝐞 𝐡𝐚𝐢 𝐩𝐫𝐨𝐯𝐚𝐭𝐨 𝐚 𝐫𝐢𝐦𝐮𝐨𝐯𝐞𝐫𝐞 𝐞́ 𝐚𝐝𝐦𝐢𝐧.`)
  ...
  async function remove() {
    ...
    const fake = {
      ...
      message: {
        contactMessage: {
          ...
          vcard: `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:y\nitem1.TEL;waid=${targetNumber}:${targetNumber}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
        }
      },
    ...
    try {
      await conn.sendMessage(m.chat, {
        text: `@${targetNumber} 𝐫𝐢𝐦𝐨𝐬𝐬𝐨 𝐝𝐚 @${senderNumber} ${reason}`,
        mentions: [target, sender, ...conn.parseMention(text)]
      }, { quoted: fake })

      await conn.groupParticipantsUpdate(m.chat, [target], 'remove')
      await logAdminAction(m.chat, sender, 'removes')
    } catch (e) {
      console.error(e)
      m.reply('ⓘ Errore durante la rimozione dell\'utente.')
    }
  }

  if (target) await remove()
}
