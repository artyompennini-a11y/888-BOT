...
const FONT_FILE = FONT_FILES.find((f) => fs.existsSync(f)) || (isWindows ? 'C:\\Windows\\Fonts\\arial.ttf' : FONT_FILES[1])
...
const renderPreview = async (name, message, profileUrl) => {
  ...
  let fontSpec = ''
  if (isWindows) {
    const winFontPath = FONT_FILE.replace(/\\/g, '\\\\').replace(':', '\\:')
    fontSpec = `fontfile='${winFontPath}'`
  } else {
    fontSpec = `fontfile='${FONT_FILE}'`
  }
  ...
  `${msgDrawtext ? ',' + msgDrawtext : ''}`

  const inputs = [ICON_PATH, profileUrl || ICON_PATH]
  
  const args = [
    '-y',
    '-protocol_whitelist', 'file,crypto,tcp,https,http,tls',
    '-i', inputs[0],
    '-i', inputs[1],
...
const getMessageText = (msg, args) => {
  ...
  if (!content) {
    ...
    else content = String(msg.text || msg.body || '').replace(/^\.(?:prova|test|screenshotgp)\b\s*/i, '').trim()
  }
