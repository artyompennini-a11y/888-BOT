...
import { isGlobalMod, isGroupMod, logModAction, roleTag, ROLES } from './moderation.js'
...
export default async function (m, conn = { user: {} }) {
  ...
  try {
    ...
    // Controllo Owner basato su global.owner (generato da normalizeOwnerList in config.js)
    const senderNum = senderJid.split('@')[0]
    const isOwner = Array.isArray(global.owner)
      ? global.owner.some(v => {
          if (!v || !v[0]) return false
          const ownerJid = v[0].includes('@') ? v[0] : v[0] + '@s.whatsapp.net'
          return ownerJid === senderJid || ownerJid.split('@')[0] === senderNum
        })
      : false
    ...
    // Moderatore: lista globale (global.mods) oppure moderatori del gruppo (.addmod)
    const isMod = (!isOwner && !isAdmin)
      ? (isGlobalMod(senderJid) || (isGroup && isGroupMod(chatJid, senderJid)))
      : false
    const roleBadge = isOwner ? roleTag(ROLES.OWNER) : isAdmin ? roleTag(ROLES.ADMIN) : isMod ? roleTag(ROLES.MOD) : '';
    ...
    const righe = [
      ...
      `${c.label('UTENTE')} ⇢ ${c.text(sender)} ${roleBadge}${(isGroup || isBot) ? ` ${c.gray('⟡ ' + getUserStatus(isBot, isBanned, c))}` : ''}`,
    ...
    if (commandText) {
      ...
      // Log moderazione: [MOD] / [ADMIN] / [OWNER] accanto a nome/numero
      if (isMod || isAdmin || isOwner) {
        const role = isOwner ? ROLES.OWNER : isAdmin ? ROLES.ADMIN : ROLES.MOD
        logModAction({
          role,
          actor: senderJid,
          action: `comando .${commandText}`,
          target: '',
          extra: chatJid.endsWith('@g.us') ? 'gruppo' : 'privato'
        })
      }
    }
