...
async function fetchF1(endpoint) {
  try {
    const res = await fetch(`https://${F1_API_HOST}/${endpoint}`, {
      headers: {
...
• .f1 piloti → elenco piloti`
    )
  }

  // PROSSIMA GARA — Grafica Premium 888
  if (cmd === "prossima") {
    await m.reply("🏎💨 Ricerca prossima gara F1...")

    const data = await fetchF1("races?season=current&type=race")
    if (!data?.response?.length)
      return m.reply("❌ Impossibile recuperare le gare.")
...
🏁 .f1 ultima`
  ...
  // CALENDARIO — Grafica Premium 888
  if (cmd === "calendario") {
    ...
    const now = new Date()
    let testo = `📅 *CALENDARIO F1 ${new Date().getFullYear()}*\n━━━━━━━━━━━━━━━━━━━━━━\n`
    ...
    for (let race of data.response) {
      ...
      const stato = new Date(race.date) < now ? "✅" : "🔜"
  ...
  // CLASSIFICA PILOTI — Grafica Premium 888
  if (cmd === "classifica") {
  ...
  // CLASSIFICA COSTRUTTORI — Grafica Premium 888
  if (cmd === "costruttori") {
    ...
    let testo = `🏭 *MONDIALE COSTRUTTORI*\n━━━━━━━━━━━━━━━━━━━━━━\n`
  ...
  // ULTIMA GARA — Grafica Premium 888
  if (cmd === "ultima") {
    ...
    let testo = `🏁 *RISULTATI ULTIMA GARA*\n${flag} ${nomeGara}\n━━━━━━━━━━━━━━━━━━━━━━\n`
  ...
  if (cmd === "piloti") {
    ...
    let testo = `🧑‍✈️ *PILOTI ${new Date().getFullYear()}*\n━━━━━━━━━━━━━━━━━━━━━━\n`
  ...
  // MENU FINALE — Grafica Premium 888
  return m.reply(
`🏎 *COMANDI F1 888*
• .f1 → prossima gara  
• .f1 calendario → stagione completa  
