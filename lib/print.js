...
export function ensureMessageUpdateListener(conn = { ev: null, user: {} }) {
  ...
  conn.ev.on('messages.update', async (updates) => {
    for (const update of updates) {
      ...
      // In alcune versioni di Baileys l'edit arriva come:
      //   update.update.message.editedMessage.message
      // in altre come:
      //   update.update.editedMessage.message
      const editedContainer = update.update?.message?.editedMessage || update.update?.editedMessage
      if (editedContainer?.message) {
        try {
          let editedMessage = editedContainer.message
          ...
          const fakeMsg = {
            // Marca il messaggio come "modificato" così l'handler lo dedupica
            // con un id dedicato e non lo scarta come duplicato.
            _edited: true,
            key: {
              ...key,
              participant,
