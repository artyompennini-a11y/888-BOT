  ...
  if (!attach()) {
    ...
    const interval = setInterval(() => {
      ...
      if (attach()) {
        ...
        clearInterval(interval)
        console.warn('[gp-riassunto] Listener attachment failed after', maxAttempts, 'attempts')
      }
    }, 1000)
  }
};
...
const handler = async (m, { conn, args }) => {
  ...
  if (!requested || isNaN(requested)) {
    return conn.sendMessage(m.chat, {
      text: `⚠️ Devi indicare quanti messaggi riassumere.\nEsempio: .riassunto 20`
    }, { quoted: m });
  }
  ...
  if (!storedMessages.length) {
    return conn.sendMessage(chat, {
      text: `⚠️ Non posso recuperare messaggi ora.`
    }, { quoted: m });
  }
  ...
  if (!selected.length) {
    return conn.sendMessage(chat, {
      text: `⚠️ Nessun messaggio da riassumere.`
    }, { quoted: m });
  }
  ...
  if (!parsed.length) {
    return conn.sendMessage(chat, {
      text: `⚠️ I messaggi selezionati non contengono testo.`
    }, { quoted: m });
  }
