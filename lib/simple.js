...
const {
  ...
  jidNormalizedUser
} = await import('@888-BOT/888baileys');
...
const AFK_NOTICE = Symbol('afkNotice');
...
function getAfkJidsForChat(afkData, chatId, conn) {
  const afkJids = new Set();
  for (const [jid, data] of Object.entries(afkData)) {
    if (!data || (data.onlyGroup && data.onlyGroup !== chatId)) continue;
    const normalized = normalizeJid(jid, conn);
    if (normalized) afkJids.add(normalized);
  }
  return afkJids;
}

function getMentionJids(message) {
  const mentions = Array.isArray(message?.mentions) ? message.mentions : [];
  const contextMentions = Array.isArray(message?.contextInfo?.mentionedJid)
    ? message.contextInfo.mentionedJid
    : [];
  return [...new Set([...mentions, ...contextMentions].filter(Boolean))];
}

function filterAfkMentions(message, afkJids, conn) {
  const originalJids = getMentionJids(message);
  const filterJids = jids => jids.filter(jid => {
    const normalized = normalizeJid(jid, conn);
    return !normalized || !afkJids.has(normalized);
  });
  const skippedJids = originalJids.filter(jid => {
    const normalized = normalizeJid(jid, conn);
    return normalized && afkJids.has(normalized);
  });

  if (Array.isArray(message?.mentions)) message.mentions = filterJids(message.mentions);
  if (message?.contextInfo && Array.isArray(message.contextInfo.mentionedJid)) {
    message.contextInfo = {
      ...message.contextInfo,
      mentionedJid: filterJids(message.contextInfo.mentionedJid)
    };
  }
...
function installAfkMentionFilter(sock) {
  if (sock.__afkMentionFilterInstalled || typeof sock.sendMessage !== 'function') return;

  const sendMessage = sock.sendMessage.bind(sock);
  const filteredSendMessage = async (chatId, message, options) => {
    if (!chatId?.endsWith?.('@g.us') || !message || message[AFK_NOTICE]) {
      return sendMessage(chatId, message, options);
    }

    const afkJids = getAfkJidsForChat(loadAfkUsers(), chatId, sock);
    const skippedJids = filterAfkMentions(message, afkJids, sock);
    const result = await sendMessage(chatId, message, options);

    if (skippedJids.length > 0) {
      const lines = await Promise.all(skippedJids.map(async jid => {
        let name = jid.split('@')[0];
        try {
          name = await sock.getName(jid) || name;
        } catch {}
        return `• ${name}`;
      }));
      ...
      await sendMessage(chatId, {
        [AFK_NOTICE]: true,
        text: `🚫 *Menzioni omesse perché AFK:*
${lines.join('\n')}`
      }, options);
    }
  ...
  Object.defineProperty(sock, 'sendMessage', {
    configurable: true,
    writable: true,
    value: filteredSendMessage
  });
  sock.__afkMentionFilterInstalled = true;
}
...
export function makeWASocket(connectionOptions, options = {}) {
  ...
  const sock = Object.defineProperties(conn, {
    ...
    sendFile: {
       ...
       * @param {import('@888-BOT/888baileys').proto.WebMessageInfo} quoted
       * @param {Boolean} ptt
...
reply: {
    ...
    async value(jid, text = '', footer = '', buffer, buttons, copy, urls, list, quoted, options) {
        ...
        if (/^https?:\/\//i.test(buffer)) {
            try {
                ...
                if (/^image\//i.test(contentType)) {
                    img = await prepareWAMessageMedia({ image: data }, { upload: conn.waUploadToServer || this?.waUploadToServer || sock?.waUploadToServer || global?.conn?.waUploadToServer });
                } else if (/^video\//i.test(contentType)) {
                    video = await prepareWAMessageMedia({ video: data }, { upload: conn.waUploadToServer || this?.waUploadToServer || sock?.waUploadToServer || global?.conn?.waUploadToServer });
                } else {
            ...
            try {
                ...
                if (/^image\//i.test(type.mime)) {
                    img = await prepareWAMessageMedia({ image: type.data }, { upload: conn.waUploadToServer || this?.waUploadToServer || sock?.waUploadToServer || global?.conn?.waUploadToServer });
                } else if (/^video\//i.test(type.mime)) {
                    video = await prepareWAMessageMedia({ video: type.data }, { upload: conn.waUploadToServer || this?.waUploadToServer || sock?.waUploadToServer || global?.conn?.waUploadToServer });
                }
            } catch (error) {
        ...
        // Convert buttons to legacy buttonMessage format when only quick_reply buttons are needed
        if (buttons && Array.isArray(buttons) && !copy && !urls && !list) {
            ...
            const hasMedia = img?.imageMessage || video?.videoMessage;
            const buttonMessage = {
                text: hasMedia ? undefined : text,
                caption: hasMedia ? text : undefined,
                footer: footer || '',
                buttons: legacyButtons,
                headerType: hasMedia ? 4 : 1,
                ...(img?.imageMessage && { image: img.imageMessage }),
                ...(video?.videoMessage && { video: video.videoMessage }),
                ...(options || {})
            };
        ...
        if (/^https?:\/\//i.test(buffer)) {
            ...
            try {
               ...
               if (/^image\//i.test(type.mime)) {
                ...
                // Convert buttons to legacy buttonMessage format when only quick_reply buttons are needed
        if (buttons && Array.isArray(buttons) && !copy && !urls) {
            const legacyButtons = buttons.map(btn => ({
                buttonId: btn[1],
                buttonText: { displayText: btn[0] },
                type: 1
...
sendList: {
  ...
  enumerable: true
    ...
    cMod: {
       ...
       * cMod
       * @param {String} jid
       * @param {import('@888-BOT/888baileys').proto.WebMessageInfo} message
       * @param {String} text
       * @param {String} sender
       * @param {*} options
       * @returns
    ...
    loadMessage: {
       ...
       * @returns {import('@888-BOT/888baileys').proto.WebMessageInfo}
       */
      value(messageID) {
    ...
    processMessageStubType: {
       ...
       * @param {import('@888-BOT/888baileys').proto.WebMessageInfo} m
       */
      async value(m) {
    ...
    switch (m.messageStubType) {
        ...
        case WAMessageStubType.GROUP_CHANGE_ICON:
            ...
            break;
        default: {
            // Nessun log a console qui: per quasi tutti i messageStubType non gestiti
            // questo ramo produceva `type: undefined` (WAMessageStubType[numero] non
            // esistente), "sporcando" la console con righe "undefined" per ogni
            // messaggio di sistema. Gli eventi reali (entrate/uscite, cambi gruppo)
            // arrivano comunque tramite ev 'group-participants.update' / 'groups.update'.
            if (global.opts?.debug || global.opts?.verbose) {
                console.log({
        ...
        for (const message of m) {
          try {
            ...
            if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
              /**
               * @type {import('@888-BOT/888baileys').proto.IContextInfo}
               */
              const context = message.message[mtype].contextInfo;
              let participant = conn.decodeJid(context.participant);
              const remoteJid = conn.decodeJid(context.remoteJid || participant);
              /**
               * @type {import('@888-BOT/888baileys').proto.IMessage}
               *
               */
              ...
              if ((remoteJid && remoteJid !== 'status@broadcast') && quoted) {
                ...
                if (!quoted[qMtype]) quoted[qMtype] = {};
