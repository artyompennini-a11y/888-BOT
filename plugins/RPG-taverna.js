let handler = async (m, { conn, text, command }) => {
    ...
    if (!rpg.classe) {
        ...
        if (classeScelta === 'guerriero') {
            ...
            msgClasse += `Scrivi uno di questi comandi per iniziare:\n\n`;
            msgClasse += `• \`.avventura guerriero\` (HP alti, Danno medio)\n`;
            msgClasse += `• \`.avventura mago\` (HP bassi, Danno alto)\n`;
            msgClasse += `• \`.avventura ladro\` (Statistiche bilanciate)\n`;
            return await conn.sendMessage(m.chat, { text: msgClasse });
        }

        return await conn.sendMessage(m.chat, {
            text: `🎉 Ti sei iscritto alla Gilda degli Avventurieri come **${rpg.classe}**!\nUsa di nuovo \`.avventura\` per partire in missione.`
        });
    }
    ...
    const eventi = [
        // Scontro Mostro
        {
            ...
            azione: (u) => {
                ...
                return `Hai sconfitto un **Goblin selvaggio** con il tuo/a *${u.arma}*!\n` +
