...
if (fs.existsSync('./owner.json')) {
  try {
    ...
    console.error("Errore owner.json:", e)
  }
} else {
...
global.isOwnerNumber = function (jid) {
  const sender = jid.replace(/[^0-9]/g, '')
  return global.owner.some(v => {
    const ownerNum = v[0].replace(/[^0-9]/g, '')
    return ownerNum === sender
  })
}
...
global.errore = '⚠️ *Errore inatteso!* Usa `.segnala` per avvisare gli owner.'
...
const reloadConfig = async () => {
  ...
  console.log(chalk.bgHex('#ff0000')(chalk.white.bold("config.js aggiornato")))
